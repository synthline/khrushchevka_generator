...
def generate_layouts():
    ...
    def overlap(layout1, layout2, min_length):
        horizontal = layout1[2] == layout2[0] and max(layout1[1], layout2[1]) < min(layout1[3], layout2[3]) - min_length
        vertical = layout1[3] == layout2[1] and max(layout1[0], layout2[0]) < min(layout1[2], layout2[2]) - min_length
    ...
    for ax, ay, bx, by in product(range(0, S[0] - min_length + 1, min_length), range(0, S[1] - min_length + 1, min_length), range(0, S[0] - min_length + 1, min_length), range(0, S[1] - min_length + 1, min_length)):
        ...
        # Check if the apartments overlap the staircase with the minimum length
        if not (overlap((a_left, a_bottom, a_right, a_top), (0, 0, S[0], S[1]), min_length) and overlap((b_left, b_bottom, b_right, b_top), (0, 0, S[0], S[1]), min_length)):
            continue
